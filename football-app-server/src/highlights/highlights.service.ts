...
export class HighlightsService {
  ...
  public getHighlights(filter: FilterHighlightsDto): HighlightDto[] {
    ...
    return highlights.filter(({ competition, homeTeam, awayTeam, matchDate }) => {
      for (const key of Object.keys(filter) as (keyof FilterHighlightsDto)[]) {
        const value = filter[key];
        if (!value) continue;

        switch (key) {
          case 'team':
            const team = (value as FilterHighlightsDto['team'])?.toLowerCase();
            if (
  ...
  public addHighlight(createDto: CreateHighlightDto) {
    ...
    return this.highlightsStorage.addHighlight(highlight);
  }
}