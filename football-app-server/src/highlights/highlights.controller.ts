...
export class HighlightsController {
  ...
  @ApiResponse({ status: HttpStatus.OK, description: 'List of highlights', type: [HighlightDto] })
  ...
  @ApiResponse({ status: HttpStatus.CREATED, description: 'The highlight has been successfully created.', type: HighlightDto })
  addHighlight(@Body() highlight: CreateHighlightDto) {
    return this.highlightsService.addHighlight(highlight);
  }

  @Get(':id')
  @ApiOperation({ summary: 'Get highlight by ID' })
  @ApiParam({ name: 'id', type: String, description: 'Highlight ID' })
  @ApiResponse({ status: HttpStatus.OK, description: 'The highlight', type: HighlightDto })
  @ApiResponse({ status: HttpStatus.NOT_FOUND, description: 'Highlight not found' })
